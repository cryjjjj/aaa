...
floatt=0
 ...
 menu.toggle(menu.my_root(),"PARACHUTE",{},"keyboard=Z\nController=R3",function()
...
--[[ old code
 menu.toggle_loop(menu.my_root(),"force ragdoll",{},"Z=on\nE=off",function(x)
    x = true 
 
  if PAD.IS_CONTROL_PRESSED(0,20)then
    util.toast("Gracefulness off")
      while x==true do
    menu.trigger_commands("grace off")
   PED.SET_PED_TO_RAGDOLL(PLAYER.PLAYER_PED_ID(), 2000, 2000, ragdoll_type,true , true , true )
     ENTITY.APPLY_FORCE_TO_ENTITY(PLAYER.PLAYER_PED_ID(), 1, 0, 1, 10, 0, -5, 0, 1, 1, 1, 1, 0, 1) do 
 util.yield(20)
 ENTITY.APPLY_FORCE_TO_ENTITY(PLAYER.PLAYER_PED_ID(), 1, 0, 1, 1, 0, -2, 0, 1, 1, 1, 1, 0, 1)  
     util.yield()
          if 
    PAD.IS_CONTROL_PRESSED(0,38) 
         then
       x=false  
       menu.trigger_commands("grace on")
        util.toast("Gracefulness on")
